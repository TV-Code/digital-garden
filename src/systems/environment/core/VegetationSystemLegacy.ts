...
import { TreeGrowthSystem, BranchNode } from './Trees/TreeGrowthSystem';
...
export class VegetationSystem {
    ...
    private treeSystem: TreeGrowthSystem;
    private growingTrees: GrowingTree[] = [];
  ...
  constructor(
    ...
    this.treeSystem = new TreeGrowthSystem();
...
draw(ctx: CanvasRenderingContext2D, time: number): void {
  ...
  this.growingTrees.forEach(tree => {
    const style = TREE_STYLES[tree.type];
    this.treeSystem.drawTree(ctx, tree.root, {
        trunk: style.trunk.color,
        highlight: [...style.trunk.color] as HSLColor
    });
});
...
plantNewTree(position: Vector2, type: TreeStyleType = 'COASTAL_PINE'): void {
    const tree: GrowingTree = {
        root: this.treeSystem.generateTree(
            position,
...
public update(time: number, deltaTime: number): void {
    ...
    this.growingTrees.forEach(tree => {
        this.treeSystem.updateGrowth(tree.root, deltaTime * 0.001);
        this.treeSystem.generatePaths(tree.root);
        tree.age += deltaTime;
    });
}
